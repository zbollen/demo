...
Note: Use Outline option on the right side of the window to see the table of content of your RMarkdown. 
...
Rewrite each of the below to use the pipe (Z: exercise done).
...
#it works also in the other side
data_amp_raw %>%
...
data_selfreport_selected_columns <- data_selfreport_renamed %>%
  select(unique_id, item, response, correct)
...
data_amp_trimmed <- read_csv("../data/raw/data_amp_raw.csv") %>%
  ...
  rename(unique_id = subject,
         ...
         block_type = blockcode, 
         rt_ms = latency) %>% 
  select(unique_id, block_type, item, trial_id, rt_ms, correct)
...
data_amp_test_trials <- data_amp_trimmed %>%
  filter(block_type == "test" & item != "instructions")
  #filter(block_type == "test" & item %in% c("positive", "prefer", "like"))
...
example_3 <- data_amp_test_trials %>%
  mutate(rt_ms = rt_ms / 1000) # latency is now in seconds rather than milliseconds
...
data_demographics_trimmed %>%
 ...
 data_demographics_age_tidy <- data_demographics_trimmed %>%
   filter(item == "age") %>%
   mutate(response = ifelse(response == "old", NA_integer_, response)) %>% 
   #mutate(response = na_if(response, "old"))
   rename(age = response)
...
# remove non Likert values and change the type of data from character to integer
...
#Other option: you can also specify a default value to return if none of the logical tests are passed with 'TRUE ~':
data_amp_tidy <- data_amp_test_trials_with_fast_trials %>%
...
#round() does not round number as we would expect, it's better to use round_half_up()
...
#class(data_demographics_age_tidy$age)
...
#TIP: Use alt and select empty lines if you want to copy paste something at several lines at a time
...
data_selfreport_scored <- data_selfreport_tidy %>%
  group_by(unique_id) %>% 
  summarize(mean_self_report = mean(response, na.rm = TRUE),
...
#full_join() is not the same as bind_col(), which is dump because it doesn't join dataset based part_id for example

```

# Create master exclude variables

Create a master `exclude_participant` variable based on having missing self report, AMP, or demographics data. [note that this is just an example: listwise exclusion of missing data is often a bad idea, and multiple imputation is usually a better but more complex option.]
...
#create a new folder for your data processed, not the same folder than raw data
