...
data_selfreport_tidy %>%
  summarize(min_response = min(response, na.rm = TRUE),
...
data_selfreport_scored %>%
  filter(n_items == 3) %>%
  summarize(mean_mean_self_report = mean(mean_self_report, na.rm = TRUE),
            sd_mean_self_report = sd(mean_self_report, na.rm = TRUE))
...
data_amp_scored <- data_amp_tidy %>%
  group_by(unique_id) %>%
  summarize(amp_score = mean(correct, na.rm = TRUE),
            proportion_too_fast_trials = mean(fast_trial, na.rm = TRUE),
            n_trials = n()) %>%
  mutate(exclude_amp = ifelse(proportion_too_fast_trials > .10, "exclude", "include"))
...
data_amp_scored %>%
  summarize(proportion_to_be_excluded = mean(exclude_amp == "exclude"))
...
data_processed <- data_demographics_age_gender_tidy %>%
  full_join(data_selfreport_scored, by = "unique_id") %>%
  full_join(data_amp_scored, by = "unique_id")
